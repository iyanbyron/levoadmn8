        ...
        <div class="layui-card-header layuiadmin-card-header-auto">
            <div class="layui-btn-group">
                ...
                @can('system.user.create')
                    <a class="layui-btn layui-btn-sm" onclick="active.openLayerCustomForm('{{route('admin.user.create')}}','添加管理员','70%','50%');">添 加</a>
                @endcan
            </div>
        </div>
            ...
            layui.use(['layer', 'table', 'form'], function () {
                ...
                table.on('tool(dataTable)', function (obj) { //注：tool是工具条事件名，dataTable是table原始容器的属性 lay-filter="对应的值"
                    ...
                    if (layEvent === 'del') {
                        ...
                        active.openLayerCustomForm('/admin/user/' + data.id + '/edit', '更新用户', '80%', '50%');
                    } else if (layEvent === 'role') {
                        active.openLayerCustomForm('/admin/user/' + data.id + '/role', '用户【' + data.name + '】分配角色', '50%', '50%');
                    } else if (layEvent === 'permission') {
                        active.openLayerCustomForm('/admin/user/' + data.id + '/permission', '用户 【' + data.name + '】分配直接权限，直接权限与角色拥有的角色权限不冲突', '98%', '50%');
                    }
                });
