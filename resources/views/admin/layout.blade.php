...
<head>
    <meta charset="utf-8">
    <title>{{ $website['title']??'LevoAdmin' }} - 主页</title>
    <meta name="keywords" content="{{ $website['keywords']??'LevoAdmin' }} - 主页">
    <meta name="description" content="{{ $website['description']??'LevoAdmin' }} - 主页">
    <meta name="renderer" content="webkit">
    <meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, minimum-scale=1.0, maximum-scale=1.0, user-scalable=0">
    <link rel="shortcut icon" type="image/x-icon" href="{{ $website['icon']??'/favicon.ico' }}">
    ...
    <div class="layui-layout layui-layout-admin">
        <div class="layui-header">
            ...
            <ul class="layui-nav layui-layout-left">
                ...
                <li class="layui-nav-item layui-hide-xs" lay-unselect>
                    <input type="text" placeholder="搜索..." autocomplete="off" class="layui-input layui-input-search" layadmin-event="serach" lay-action="/admin/member/data?page=1&limit=9&model=member&id=&username=">
                </li>
            ...
            <div class="layui-side-scroll">
                <div class="layui-logo" lay-href="{{route('admin.index')}}">
                    ...
                    @if(auth()->user()->user_type==0)
                        <span>{{ $website['webname']??'后台管理系统' }}</span>
                    @else
                        <span>{{ $website['webname']??'代理管理系统' }}</span>
                    @endif
                </div>
                ...
                <ul class="layui-nav layui-nav-tree" lay-shrink="all" id="LAY-system-side-menu" lay-filter="layadmin-system-side-menu">
                    <li data-name="home" class="layui-nav-item layui-nav-itemed">
                        ...
                        @if(auth()->user()->user_type==0)
                            <a href="javascript:;" lay-href="{{route('admin.index')}}" lay-tips="主页" lay-direction="2">
                                <i class="layui-icon layui-icon-home"></i>
                                <cite>主页</cite>
                            </a>
                        @else
                            <a href="javascript:;" lay-href="{{route('admin.agent_index')}}" lay-tips="主页" lay-direction="2">
                                <i class="layui-icon layui-icon-home"></i>
                                <cite>首页</cite>
                            </a>
                        @endif
                    ...
                    @foreach($menus as $menu)
                        @can($menu->name)
                            <li data-name="{{$menu->name}}" class="layui-nav-item">
                                ...
                                @else
                                    <a href="javascript:;" lay-href="{{ route($menu->route) }}" lay-tips="{{$menu->display_name}}" lay-direction="2">
                                        <i class="layui-icon {{$menu->icon??''}}"></i>
                                        <cite>{{$menu->display_name}}</cite>
                                    </a>
                                @endif
        ...
        <div class="layui-body" id="LAY_app_body">
            <div class="layadmin-tabsbody-item layui-show">
                ...
                @else
                    <iframe src="{{route('admin.agent_index')}}" frameborder="0" class="layadmin-iframe"></iframe>
                @endif
            </div>
        </div>
        @if(isset($website['copyright']))
            ...
            <style> .layui-layout-admin .layui-body {
                    bottom: 50px;
                } </style>
    @endif
    <!-- 辅助元素，一般用于移动设备下遮罩 -->
