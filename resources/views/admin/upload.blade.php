        ...
        layui.use('upload', function () {
            ...
            upload.render({
                ...
                , before: function (obj) {
                    //预读本地文件示例，不支持ie8
                    obj.preview(function (index, file, result) {
                        $('#demo2').append('<img src="' + result + '" alt="' + file.name + '" class="layui-upload-img" style="max-height: 100px;max-width: 100px;padding-left: 10px;">')
                    });
                }
                , done: function (res) {
                    if (res.status == 1) { //上传成功
                        /*document.getElementById('ads_pic').value = res.file_name;
                        var fileupload = $("#ads_pic");
                        fileupload.attr("value", res.file_name.src);
            ...
            var demoListView = $('#demoList')
                ...
                , done: function (res, index, upload) {
                    if (res.status) { //上传成功
                        /*document.getElementById('ads_pic').value=res.file_name;
                        var fileupload = $("#ads_pic");
                        fileupload.attr("value",res.file_name);
                        console.log(fileupload.attr("value"));*/

                        //$('#pre_img').attr('src', domian + res.msg);
                        //给img隐藏域赋值，用于提交保存
                        $('input[name="ads_pic"]').val(res.msg);
