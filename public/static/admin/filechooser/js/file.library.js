(function ($, window, document, undefined) {
    ...
    FileLibrary.prototype = {
        ...
        showLibraryModal: function () {
            var _this = this;
            _this.getJsonData({type: _this.options.type, group_id: -1, page: 1}, function (data) {
                data.is_default = true;
                layer.open({
                    type: 1
                    , id: _this.options.layerId
                    , title: _this.options.type == 'image' ? '图片库' : '文件库'
        ...
        uploadImagesEvent: function () {
            ...
            var loadIndex = null;
            // 文件大小
            var maxSize = _this.options.maxsize || 2;
            var _accept = _this.options.type == 'video' ? {
                title: 'video'
                , extensions: _this.options.extensions || 'mp4,flv,mov'
            } : {
                title: 'Images'
                , extensions: _this.options.extensions || 'gif,jpg,jpeg,bmp,png'
                , mimeTypes: 'image/*'
            };
            // 初始化Web Uploader
            ...
            uploader.on('error', function (type) {
                if (type === 'F_DUPLICATE') {
                    ...
                    layer.msg('文件大小不可超过' + maxSize + 'm 哦！换个小点的文件吧！');
                }
            });
            // 文件上传前触发,添加附带参数
            uploader.on('uploadBeforeSend', function (obj, data) {
                console.log(_this.options.token)
                data.group_id = _this.getCurrentGroupId();
                data.filetype = _this.options.type || 'image';
                data._token = _this.options.token;
            });
        ...
        getJsonData: function (params, success) {
            ...
            // 获取文件库列表
            $.get(FileList, params, function (result) {
                layer.close(loadIndex);
                if (result.code === 1) {
